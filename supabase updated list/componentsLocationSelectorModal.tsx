...
import { X, ChevronLeft, ChevronRight, CheckCircle, MapPin, AlertCircle, Loader2, Globe } from 'lucide-react';
...
const translations = {
  en: {
    ...
    any_district: 'Any District in' // NEW
  },
  so: {
    ...
    any_district: 'Dhammaan Degmooyinka' // NEW
  },
  ar: {
        ...
        <div className="flex-1 overflow-y-auto custom-scrollbar p-2">
          {isLoading ? (
            ...
            <div className="space-y-1 p-2">
              ...
              {/* ======================================================== */}
              {/* NEW UX FEATURE: "Any District" Button (Skips micro-selection) */}
              {/* ======================================================== */}
              {currentStep === Step.DISTRICT && (
                <button
                  onClick={(e) => { 
                    e.stopPropagation(); 
                    handleFinish(null); // Passing null skips the district requirement
                  }}
                  className="w-full flex items-center justify-between p-4 mb-3 rounded-2xl bg-[#0065eb]/10 border border-[#0065eb]/20 hover:bg-[#0065eb]/20 transition-all group text-left"
                >
                  <div className="flex items-center gap-3">
                    <Globe size={18} className="text-[#0065eb]" />
                    <span className="font-black text-[#0065eb]">{t.any_district} {selectedCity?.name}</span>
                  </div>
                  <CheckCircle size={18} className="text-[#0065eb]" />
                </button>
              )}
              ...
              {items.map(item => (
                ...
                >
                  ...
                  {currentStep === Step.DISTRICT ? (
                    <MapPin size={18} className="text-slate-300 group-hover:text-[#0065eb] transition-colors" />
                  ) : (
