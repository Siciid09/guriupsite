...
export async function GET(request: Request) {
  try {
    ...
    const amenitiesParam = searchParams.get('amenities'); // Comma-separated list
    const requestAgentId = searchParams.get('agentId'); // For dashboard fetching
    ...
    // Batch fetch agents to populate Agent Names and Photos
    ...
    let filteredData = mergedData.filter(p => {
      // Search Query (q) matching Title, Description, City, Area, Agent Name, Type, Amenities, Price
      if (q) {
...
export async function PATCH(request: Request) {
  try {
    ...
    if (checkError || !checkData || checkData.agentId !== uid) {
