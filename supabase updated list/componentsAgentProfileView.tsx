...
export default function AgentProfileView() {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        ...
        // 1. Fetch Agent Profile (via our new API which handles Slug/ID fallback)
        ...
        // Ensure defaults for missing fields to keep UI happy
        ...
        // 2. Fetch Agent's Properties (via our updated Properties API)
        const propsRes = await fetch(`/api/properties`);
        if (propsRes.ok) {
          const apiProperties = await propsRes.json();
          
          // Filter properties client-side to only show this specific agent's properties
          const agentProperties = apiProperties.filter((p: any) => p.agentId === apiAgent.id);
          
          const formattedProperties: Property[] = agentProperties.map((p: any) => {
            // Format location string safely from the normalized API object
            ...
            return {
                ...
                area: p.area || 0,
                planTier: p.planTier,
                isArchived: false, // The properties API automatically filters archived ones out
            };
          });
 ...
 const handleContactAction = (type: 'call' | 'whatsapp') => {
  ...
  const submitRating = async () => {
    ...
    try {
        // Changed to use standard API route instead of direct Firestore write
