...
export default async function HomePage() {
  ...
  // =========================================================
  try {
    // Fetch everything concurrently for max speed
    const [featProps, lateProps, featHotels, lateHotels] = await Promise.all([
      getFeaturedProperties().catch(() => []),
      getLatestProperties().catch(() => []),
      getFeaturedHotels().catch(() => []),
      getAllHotels().catch(() => []) // Assuming you use getAllHotels for latest
    ]);

    featuredProperties = featProps || [];
    latestProperties = lateProps || [];
    featuredHotels = featHotels || [];
    latestHotels = lateHotels || [];
  } catch (error) {
