...
import { 
  ...
  Globe, Info, X, Database, Activity, MapPin, Video, Utensils, Calendar
} from 'lucide-react';
...
// 1. TYPES & INTERFACES
// ==========================================
type TabType = 'users' | 'agents' | 'hotels' | 'properties' | 'restaurants' | 'bookings';

// ==========================================
// 2. HELPER UTILITIES
// ==========================================
const parseValueDate = (timestamp: any): string => {
...
const formatValue = (val: any): string => {
  ...
  if (typeof val === 'object') {
    if (val.seconds) return parseValueDate(val);
...
export default function AdminDashboard() {
  ...
  // ------------------------------------------
  // SECURITY GUARD: API TOKEN AUTH CHECK
  // ------------------------------------------
  useEffect(() => {
    const verifyAccess = async () => {
      ...
      try {
        ...
        const res = await fetch('/api/admin?resource=users&limit=1', {
  ...
  const handleAction = async (id: string, type: string, action: string, payload: any = {}) => {
    ...
    setData(prev => prev.map(item => {
      if (item.id === id) {
        ...
        if (action === 'unban') return { ...item, status: 'active', isBanned: false };
      }
      return item;
    }));
    ...
    if (selectedItem?.id === id) {
      setSelectedItem((prev: any) => ({ 
        ...prev, 
        planTier: payload.plan || prev.planTier, 
        status: action === 'ban' ? 'banned' : action === 'unban' ? 'active' : prev.status 
      }));
    }
  ...
  const openChat = (item: any) => {
    setChatConfig({
      ...
      recipientName: item.name || item.agencyName || item.title || item.hotelName || 'User'
    });
  };
  ...
  const filteredData = data.filter(item => 
    (item.name || item.agencyName || item.title || item.email || item.hotelName || item.roomTypeName || '').toLowerCase().includes(searchTerm.toLowerCase())
  );
    ...
    <div className="min-h-screen bg-slate-50 md:pl-64 flex flex-col font-sans text-slate-900 selection:bg-blue-200">
      ...
      <aside className="fixed left-0 top-0 h-full w-64 bg-slate-950 text-slate-300 p-6 hidden md:flex flex-col z-40 border-r border-slate-800">
        ...
        <nav className="flex-1 space-y-2">
          {[
            ...
            { id: 'agents', label: 'Agents', icon: <Briefcase size={20}/> },
            { id: 'hotels', label: 'Hotels', icon: <Building size={20}/> },
            ...
            <button 
              ...
              className={`w-full flex items-center gap-4 px-5 py-3 rounded-2xl font-bold transition-all duration-300 ${
                activeTab === tab.id 
                  ? 'bg-[#0065eb] text-white shadow-lg shadow-blue-900/50 scale-[1.02]' 
                  : 'hover:bg-slate-800 hover:text-white'
              }`}
            >
              {tab.icon} <span className="text-sm tracking-wide">{tab.label}</span>
            </button>
          ))}
        </nav>
      ...
      <header className="sticky top-0 z-30 bg-white/80 backdrop-blur-xl border-b border-slate-200 px-8 py-5 flex flex-col sm:flex-row gap-4 items-center justify-between">
        <div className="relative w-full md:w-[28rem]">
          ...
          <input 
            type="text" 
            placeholder={`Search ${activeTab} by name, email, or title...`} 
            value={searchTerm}
      ...
      <main className="p-8 max-w-[1600px] flex-1">
        <div className="mb-8 flex items-end justify-between">
          <div>
            ...
            <p className="text-slate-500 font-medium mt-2 flex items-center gap-2">
              <Database size={16}/> Showing {filteredData.length} records via API
            </p>
          </div>
        </div>
        ...
        {loading ? (
          <div className="flex flex-col items-center justify-center h-96">
            ...
            <p className="font-bold text-slate-400 animate-pulse">Fetching records from server...</p>
          </div>
              ...
              <div key={item.id} className="bg-white p-6 rounded-[2rem] border border-slate-200 shadow-sm hover:shadow-xl hover:-translate-y-1 transition-all duration-300 flex flex-col relative overflow-hidden group">
                ...
                <div className="flex justify-between items-start mb-5">
                  <div className={`w-14 h-14 rounded-2xl flex items-center justify-center shadow-inner ${
                    ...
                    {activeTab === 'restaurants' && <Utensils size={28}/>}
                ...
                <h3 className="font-black text-slate-900 text-xl truncate mb-1">
                  {item.name || item.agencyName || item.title || item.hotelName || item.roomTypeName || 'Unnamed Record'}
                </h3>
                
                <div className="flex items-center gap-1.5 text-xs text-slate-500 font-medium mb-5 truncate">
                  <MapPin size={12}/> {item.city || item.location?.city || item.email || item.phone || 'No location set'}
                </div>
                    ...
                    <button 
                      onClick={() => {
                        if (item.planTier === 'pro') {
                          handleAction(item.id, resourceTypeMap[activeTab], 'promote_plan', {plan: 'free'});
                        } else {
                          setPlanModal({isOpen: true, item});
                        }
                      }} 
            ...
            <div className="bg-slate-900 text-white p-6 sm:p-8 flex justify-between items-start relative overflow-hidden shrink-0">
              ...
              <div className="relative z-10 flex gap-5 items-center w-full">
                <div className="w-20 h-20 bg-white/10 rounded-2xl flex items-center justify-center backdrop-blur-xl border border-white/20 shadow-inner shrink-0">
                  ...
                  {activeTab === 'restaurants' && <Utensils size={36}/>}
                  {activeTab === 'bookings' && <Calendar size={36}/>}
                </div>
                <div className="flex-1 pr-4">
                  <div className="flex items-center gap-3 mb-1">
                    <h2 className="text-3xl font-black truncate">{selectedItem.name || selectedItem.agencyName || selectedItem.title || selectedItem.hotelName || 'Unknown Entity'}</h2>
                  ...
                  <p className="text-sm font-medium text-blue-200 flex items-center gap-2 opacity-80 font-mono">
                    <Database size={14}/> ID: {selectedItem.id}
                  </p>
                  <p className="text-xs font-bold text-slate-400 mt-2 uppercase tracking-widest flex items-center gap-1">
                    Created / Joined: {parseValueDate(selectedItem.createdAt || selectedItem.joinDate)}
                  </p>
                </div>
              </div>
            ...
            <div className="flex-1 overflow-y-auto custom-scrollbar bg-slate-50/50">
              <div className="bg-white border-b border-slate-100 p-6 flex flex-wrap gap-3 sticky top-0 z-20 shadow-sm">
                ...
                <button onClick={() => openChat(selectedItem)} className="flex-1 bg-[#0065eb]/10 hover:bg-[#0065eb] hover:text-white text-[#0065eb] py-3 px-4 rounded-xl text-sm font-black uppercase tracking-wide flex items-center justify-center gap-2 transition-all min-w-[140px]">
                  <MessageCircle size={16} /> Firebase Chat
                </button>
      ...
      {/* --- INTEGRATED FIREBASE CHAT COMPONENT --- */}
      <SharedChatComponent 
