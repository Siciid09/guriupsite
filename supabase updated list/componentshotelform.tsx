...
import { auth } from '../app/lib/firebase'; // Keep Auth client-side only for token generation
import { 
...
export default function HotelForm({ hotelId }: HotelFormProps) {
  ...
  useEffect(() => {
    const fetchContext = async () => {
      try {
        ...
        const userRes = await fetch(`/api/user?uid=${user.uid}`, {
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Upload new image files securely through the API upload route
      let finalImageUrls = [...existingImages];
      for (const img of newImages) {
        const formData = new FormData();
        formData.append('file', img.file);
        formData.append('folder', 'hotel_images');
