...
import { auth } from '../../lib/firebase'; // Keep Auth client-side for token generation if needed
import { 
  MapPin, 
  Bed, 
  Bath, 
...
function SearchContent() {
  ...
  // --- FETCH DATA VIA SECURE NEXT.JS API INSTEAD OF DIRECT FIREBASE ---
  useEffect(() => {
    async function fetchSearchData() {
      ...
      try {
        ...
        const queryType = searchParams.get('type') || searchParams.get('roomType');

        // 2. Build Query Parameters for the API Route
        const params = new URLSearchParams();
        params.append('mode', currentMode);
        if (cityParam && cityParam !== 'All Cities') params.append('city', cityParam);
        if (queryType && queryType !== 'Any Type') params.append('type', queryType);
        if (priceParam && priceParam !== 'Any Price') params.append('price', priceParam);

        const endpoint = currentMode === 'hotel' ? `/api/hotels?${params.toString()}` : `/api/properties?${params.toString()}`;
        ...
        if (currentMode === 'hotel') {
          const rawHotels = data.success ? data.hotels : (Array.isArray(data) ? data : []);
          const formattedHotels = rawHotels.map((h: any) => {
            let amenitiesArray: string[] = [];
            if (Array.isArray(h.amenities)) {
              amenitiesArray = h.amenities;
            } else if (typeof h.amenities === 'object' && h.amenities !== null) {
              if (h.amenities.hasWifi) amenitiesArray.push('Wi-Fi');
              if (h.amenities.hasPool) amenitiesArray.push('Pool');
              if (h.amenities.hasGym) amenitiesArray.push('Gym');
              if (h.amenities.hasRestaurant) amenitiesArray.push('Restaurant');
              if (h.amenities.hasParking) amenitiesArray.push('Parking');
            }
            return {
              id: h.id || h._id,
              ...h,
          ...
          const rawProps = data.success ? data.properties : (Array.isArray(data) ? data : []);
          const formattedProps = rawProps.map((p: any) => {
            const beds = p.bedrooms ?? p.features?.bedrooms ?? 0;
            const baths = p.bathrooms ?? p.features?.bathrooms ?? 0;
            const size = p.size ?? p.area ?? p.features?.area ?? p.features?.size ?? 0;
            ...
            return {
              ...
              hasDiscount: (p.hasDiscount === true) && discountPrice > 0,
