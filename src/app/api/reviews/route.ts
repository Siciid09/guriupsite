...
export async function GET(request: Request) {
  try {
    ...
    // 🔥 NEW: Fetch directly from the dedicated menu_items table
    if (entity === 'menu_item') {
...
export async function POST(request: Request) {
  try {
    ...
    // 🔥 NEW: Insert directly into dedicated menu_items table
    if (entity === 'menu_item') {
      ...
      // Clean empty strings ("") to null for strict SQL number columns
...
export async function PATCH(request: Request) {
  try {
    ...
    // 🔥 NEW: Update the dedicated menu_items table directly
    if (entity === 'menu_item') {
      // Clean empty strings ("") to null for strict SQL number columns
      const safeNumber = (val: any) => (val === "" || val === null || val === undefined) ? null : Number(val);
...
export async function DELETE(request: Request) {
  try {
    ...
    // 🔥 NEW: Delete directly from the dedicated menu_items table
    if (entity === 'menu_item') {
      const { error } = await supabaseAdmin
        .from('menu_items')
        .delete()
