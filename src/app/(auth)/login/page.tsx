...
export default function LoginPage() {
    ...
    try {
      ...
      if (userDoc.exists()) {
        ...
        if (userData.role === 'agent') {
         ...
         // Fallback if user doc is missing (shouldn't happen normally)
         router.push('/');
      }

    } catch (err: any) {
      console.error("Login Error:", err);
      // Simplified error messages for demo
  ...
  const handleGoogleLogin = async () => {
    ...
    try {
      ...
      if (!userDoc.exists()) {
        // New Google user - create doc with default 'user' role
  ...
  return (
    ...
    <div className="min-h-screen flex bg-white">
      ...
      <div className="hidden lg:block relative w-0 lg:w-1/2 bg-slate-900 overflow-hidden">
        <Image 
          // Using a high-quality Unsplash image fitting for luxury property/GuriUp
