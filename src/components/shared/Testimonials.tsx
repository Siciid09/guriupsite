        ...
        <div className="relative max-w-2xl mx-auto h-72">
          {testimonialsData.map((testimonial, index) => (
            ...
            >
              ...
              <h4 className="font-bold text-lg text-black">{testimonial.name}, <span className="font-normal text-gray-500">{testimonial.location}</span></h4>
            </div>
          ))}
        </div>
        <div className="flex justify-center gap-2 mt-4">
          {testimonialsData.map((_, index) => (
            <button
              ...
              className={`w-2.5 h-2.5 rounded-full transition-all duration-300 ${index === currentIndex ? 'bg-[#0164E5] w-6' : 'bg-gray-300'}`}
